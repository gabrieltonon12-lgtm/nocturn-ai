...
export default function Dashboard() {
  ...
  const [reportingVideoId, setReportingVideoId] = useState<string>('')
  ...
  useEffect(() => {
    ...
    const token = t
    fetch('/api/videos', { headers: { Authorization: 'Bearer ' + token } }).then(r=>r.json()).then(d=>setVideos(d.videos||[]))
    fetch('/api/rewards', { headers: { Authorization: 'Bearer ' + token } }).then(r=>r.json()).then(d=>setRewards(d.rewards||[]))
  }, [])
    ...
    try {
      ...
      setTimeout(async () => {
        ...
        if (res.ok && data.video) {
          ...
          // Refresh rewards after generating
  ...
  const handleClaimReward = async (rewardId: string) => {
    ...
    } catch { alert('Erro ao resgatar reward') }
    setClaimingId('')
  }
  ...
  return (
    <>
      ...
      {/* REWARD TOAST */}
      {rewardToast && (
        <div style={{ position:'fixed',top:'20px',right:'20px',zIndex:9999,background:'linear-gradient(135deg,#ff3c5c,#ff6b35)',color:'#fff',padding:'14px 20px',borderRadius:'12px',fontWeight:700,fontSize:'14px',boxShadow:'0 8px 32px rgba(255,60,92,.4)',animation:'slideIn .3s ease' }}>
          {rewardToast}
        </div>
      )}
          ...
          <nav style={{ padding:'12px 8px',flex:1,overflowY:'auto' }}>
            {[
              ...
              <div key={item.id} onClick={() => setView(item.id)} style={{ display:'flex',alignItems:'center',gap:'10px',padding:'9px 10px',borderRadius:'7px',cursor:'pointer',fontSize:'13px',marginBottom:'2px',background:view===item.id?'rgba(255,60,92,.12)':'transparent',color:view===item.id?'#ff3c5c':'#8892a4',fontWeight:view===item.id?700:400,position:'relative' }}>
                ...
                {item.badge && <span style={{ fontSize:'9px',padding:'2px 7px',borderRadius:'10px',fontWeight:700,fontFamily:'monospace',background:item.badgeRed?'rgba(255,60,92,.9)':item.badge==='IA'?'#00d084':'rgba(255,60,92,.3)',color:item.badgeRed||item.badge==='IA'?'#fff':'#ff3c5c' }}>{item.badge}</span>}
              </div>
            ))}
            {user.role === 'admin' && (
              <>
              ...
              <div>
                ...
                <div style={{ background:'#0e1219',border:'1px solid #1e2840',borderRadius:'14px',padding:'24px',marginBottom:'16px' }}>
                    ...
                    <div style={{ display:'grid',gridTemplateColumns:'repeat(4,1fr)',gap:'6px' }}>
                      {QUICK_PROMPTS.map((qp,i) => (
                        <div key={i} onClick={() => setPrompt(qp.text)} style={{ background:prompt===qp.text?'rgba(255,60,92,.15)':'#141920',border:`1px solid ${prompt===qp.text?'rgba(255,60,92,.5)':'#1e2840'}`,borderRadius:'8px',padding:'8px 10px',cursor:'pointer',transition:'all .15s' }}
                          onMouseEnter={e => { (e.currentTarget as HTMLElement).style.borderColor='rgba(255,60,92,.4)'; (e.currentTarget as HTMLElement).style.background='rgba(255,60,92,.08)' }}
                          onMouseLeave={e => { if(prompt!==qp.text){(e.currentTarget as HTMLElement).style.borderColor='#1e2840';(e.currentTarget as HTMLElement).style.background='#141920'} }}>
                    ...
                    <div style={{ display:'flex',flexDirection:'column',gap:'6px' }}>
                      ...
                      <div style={{ display:'flex',gap:'6px',flexWrap:'wrap',marginTop:'4px' }}>
                        {['youtube','tiktok','instagram','shorts'].map(p => (
                          <div key={p} onClick={() => togglePlat(p)} style={{ padding:'6px 10px',borderRadius:'16px',fontSize:'11px',fontWeight:600,cursor:'pointer',userSelect:'none' as any,border:`1px solid ${platforms.includes(p)?'#ff3c5c':'#1e2840'}`,color:platforms.includes(p)?'#ff3c5c':'#8892a4',background:platforms.includes(p)?'rgba(255,60,92,.08)':'transparent' }}>
                  ...
                  <div style={{ display:'flex',gap:'12px',alignItems:'center' }}>
                    ...
                    {generating && (
                      <>
                        <div style={{ flex:1,height:'4px',background:'#1e2840',borderRadius:'2px',overflow:'hidden' }}>
                          <div style={{ height:'100%',background:'linear-gradient(90deg,#ff3c5c,#ff6b35)',borderRadius:'2px',transition:'width .5s',width:progress+'%' }}/>
                        </div>
                        <span style={{ fontSize:'11px',color:'#8892a4',fontFamily:'monospace',minWidth:'34px' }}>{progress}%</span>
                      </>
                    )}
                  </div>

                  {logs.length > 0 && (
                    <div ref={logRef} style={{ background:'#141920',border:'1px solid #1e2840',borderRadius:'8px',padding:'12px',fontFamily:'monospace',fontSize:'11px',color:'#f0f2f8',maxHeight:'120px',overflowY:'auto',marginTop:'14px',lineHeight:1.7 }}>
                      {logs.map((l,i) => (
                        <div key={i}>
                          <span style={{ color:'#4a5568',marginRight:'8px' }}>[{String(i*3).padStart(2,'0')}s]</span>
                          <span style={{ color:l.includes('DONE')?'#00d084':'#ff6b35',marginRight:'6px' }}>{l.split(' ')[0]}</span>
            ...
            {view === 'videos' && (
              <div>
                ...
                {videos.length === 0 ? (
                  ...
                  <VideoGrid videos={videos} onSelect={setSelectedVideo} onReportViews={(id) => setReportingVideoId(id)}/>
                )}
              </div>
            )}
                    ...
                    <div key={r.id} style={{ background:r.unlocked?'rgba(0,208,132,.03)':'#0e1219',border:`1px solid ${r.unlocked?'rgba(0,208,132,.2)':r.eligible?'rgba(255,60,92,.3)':'#1e2840'}`,borderRadius:'12px',padding:'18px',opacity:r.unlocked?0.7:1 }}>
                      ...
                      {!r.unlocked && (
                        <div style={{ marginBottom:'12px' }}>
                          <div style={{ display:'flex',justifyContent:'space-between',fontSize:'10px',color:'#4a5568',marginBottom:'4px',fontFamily:'monospace' }}>
                            <span>Progresso</span>
                            <span>{r.progress >= 1000 ? `${(r.progress/1000).toFixed(0)}k` : r.progress} / {r.target >= 1000 ? `${(r.target/1000).toFixed(0)}k` : r.target}</span>
                          </div>
                          <div style={{ height:'4px',background:'#1e2840',borderRadius:'2px',overflow:'hidden' }}>
                            <div style={{ height:'100%',width:`${Math.min(100, Math.round(r.progress/r.target*100))}%`,background:r.eligible?'linear-gradient(90deg,#ff3c5c,#ff6b35)':'linear-gradient(90deg,#4a5568,#6b7280)',borderRadius:'2px',transition:'width .5s' }}/>
                          </div>
                        </div>
                      )}
      ...
      {/* VIDEO MODAL */}
      {selectedVideo && (
        <div onClick={() => setSelectedVideo(null)} style={{ position:'fixed',inset:0,background:'rgba(0,0,0,.85)',display:'flex',alignItems:'center',justifyContent:'center',zIndex:1000,padding:'20px' }}>
...
function VideoGrid({ videos, onSelect, onReportViews }: { videos: any[], onSelect: (v: any) => void, onReportViews?: (id: string) => void }) {
  return (
    <div style={{ display:'grid',gridTemplateColumns:'repeat(auto-fill,minmax(200px,1fr))',gap:'12px' }}>
      {videos.map((v: any) => (
        <div key={v.id} style={{ background:'#0e1219',border:'1px solid #1e2840',borderRadius:'10px',overflow:'hidden',cursor:'pointer' }}
