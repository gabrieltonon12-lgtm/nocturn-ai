...
export default function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  let d: any; try { d = jwt.verify(token, JWT) } catch { return res.status(401).json({ error: 'invalido' }) }
  if (d.role !== 'admin') return res.status(403).json({ error: 'Acesso negado' })
  const users = getUsers(); const videos = getVideos()
  ...
  res.status(200).json({ stats: { mrr, totalUsers: users.length, videosToday, churn: 2.1 }, users: users.map(({ password: _, ...u }: any) => u) })
}